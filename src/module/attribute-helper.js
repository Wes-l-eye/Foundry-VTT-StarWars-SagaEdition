...
export function inheritableItems(entity) {
  ...
  possibleInheritableItems.push(
    ...filterItemsByType(entity.items || [], [
      'background',
      'destiny',
      'trait',
      'feat',
  ...
  while (shouldRetry) {
    ...
    possibleInheritableItems = possibleInheritableItems.filter(
      (possible) => !entity.system.inheritableItems.includes(possible),
    );
  }
...
export function getResolvedSize(entity, options) {
  ...
  let sizeIndex = toNumber(
    getInheritableAttribute({ entity, attributeKey: 'sizeIndex', reduce: 'MAX', recursive: true }),
  );
  let sizeBonus = toNumber(
    getInheritableAttribute({ entity, attributeKey: 'sizeBonus', reduce: 'SUM', recursive: true }),
  );

  let damageThresholdEffectiveSize = toNumber(
    getInheritableAttribute({
      entity,
...
export function getInheritableAttribute(data = {}) {
  ...
  if (Array.isArray(data.attributeKey)) {
    ...
    if (document.system?.modes) {
      unfilteredAttributes.push(
        ...extractModeAttributes(
          document,
          Object.values(document.system.modes).filter((mode) => mode && mode.isActive) || [],
        ),
      );
    }
  ...
  if (!data.recursive && data.parent) {
    values = values.filter((attr) => {
      ...
      if (
        attr.parentPrerequisite &&
        meetsPrerequisites(parent, attr.parentPrerequisite, { attributeKey: data.attributeKey }).doesFail
      ) {
        return false;
      }
...
export function extractModeAttributes(entity, activeModes) {
  ...
  for (let mode of activeModes) {
    ...
    values.push(
      ...extractModeAttributes(entity, Object.values(mode.modes || {}).filter((mode) => mode && mode.isActive) || []),
    );
  }
  return values;
}
