...
export function meetsPrerequisites(target, prereqs, options = {}) {
  ...
  for (let prereq of prereqs) {
    switch (prereq.type.toUpperCase()) {
      ...
      case 'TRAINED SKILL':
        if (
          Object.entries(system.skills).filter(
            (skill) => skill[0].toLowerCase() === prereq.requirement.toLowerCase() && skill[1].trained,
          ).length === 1
        ) {
          successList.push({ prereq, count: 1 });
          continue;
      ...
      case 'FEAT':
        ...
        if (prereq.requirement.toLowerCase().includes('(any)')) {
          ...
          let possibleFeats = exoticMeleeWeapons?.map((w) =>
            prereq.requirement.replace('(Exotic Melee Weapons)', `(${w})`),
          );
      ...
      case 'CLASS':
        ...
        break;
      case 'TRAIT':
        let filteredTraits = filterItemsByType(getItems(target), 'trait').filter(
          (feat) => feat.finalName === prereq.requirement,
        );
      ...
      case 'SPECIAL_QUALITY':
        let filteredSpecialQualities = filterItemsByType(getItems(target), 'beastQuality').filter(
          (feat) => feat.finalName === prereq.requirement,
        );
        ...
        break;
      case 'SPECIES_TYPE':
        let filteredSpeciesTypes = filterItemsByType(getItems(target), 'beastType').filter(
          (feat) => feat.finalName === prereq.requirement,
        );
        ...
        break;
      case 'BEAST_ATTACK':
        let filteredBeastAttacks = filterItemsByType(getItems(target), 'beastAttack').filter(
          (feat) => feat.name === prereq.requirement,
        );
      ...
      case 'TALENT':
        ...
        let filteredTalents = ownedTalents.filter((talent) => {
          let actsAs =
            getInheritableAttribute({ entity: talent, recursive: true, attributeKey: 'actsAs', reduce: 'VALUES' }) ||
            [];

          return (
            talent.finalName === prereq.requirement ||
            talent.system.possibleProviders.includes(prereq.requirement) ||
            talent.system.talentTree === prereq.requirement ||
            actsAs.includes(prereq.requirement)
          );
        });
