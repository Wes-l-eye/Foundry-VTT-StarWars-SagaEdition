import { toShortAttribute } from '../util.js';
...
export function generateVehicleAttacks(actor) {
  let map = {};
  actor.system.equippedIds.forEach((equippedId) => {
    map[equippedId.id] = actor.getCrewByPosition(equippedId.position, equippedId.slot)?._id;
  });
  ...
  attacks.push(
    ...actor
      ...
      .map(
        (weapon) => new Attack(map[weapon._id], weapon._id, weapon.parentId, actor.parent?.id, { actor: actor.items }),
      ),
  );
...
export function generateAttacks(actor) {
  ...
  let weaponIds = equippedItems.filter((item) => 'weapon' === item.type).map((item) => item.id);

  let beastAttackIds = actor.naturalWeapons.filter((item) => 'beastAttack' === item.type).map((item) => item.id);
  ...
  attacks.push(
    ...items.map((item) => new Attack(actor.id, item._id, item.parentId, actor.parent?.id, { actor: actor.items })),
  );
  return attacks;
}
...
export function getPossibleProficiencies(actor, weapon) {
  ...
  let descriptors =
    exoticWeaponTypes.length > 0 ? exoticWeaponTypes : [weapon.name, weapon.system.subtype, weapon.type];
...
export function getFocusAttackBonuses(actor, weaponTypes) {
  let bonuses = [];
  let weaponFocus = explodeProficiencies(
    getInheritableAttribute({
      entity: actor,
  ...
  let greaterWeaponFocus = explodeProficiencies(
    getInheritableAttribute({
      entity: actor,
