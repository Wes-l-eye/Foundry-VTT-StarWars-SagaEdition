...
export function warningsFromActor(actor) {
  ...
  if (!actor.species) {
    warnings.push(
      `<span data-action="compendium" data-type="Item" data-filter="-type:species">Please Select a Species</span>`,
    );
  }
  if (!actor.classes) {
  ...
  for (let feat of actor.system.inactiveProvidedFeats || []) {
    warnings.push(
      `<span>The ${feat.finalName} feat is provided but cannot be added because of missing prerequisites: ${feat.system.prerequisite.text}</span>`,
    );
  }

  return warnings;
}
export function errorsFromActor() {
  let errors = [];
