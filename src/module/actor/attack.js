...
export function appendNumericTerm(value, flavor) {
  ...
  return [num > -1 ? plus() : minus(), new NumericTerm({ number: Math.abs(num), options: getDieFlavor(flavor) })];
}
...
export class Attack {
  ...
  get attackRoll() {
    ...
    return new SWSERollWrapper(Roll.fromTerms(terms.filter((term) => !!term)));
  }
  ...
  get damageRoll() {
    ...
    let heroicClassLevels = (actor?.items || []).filter(
      (item) =>
        item.type === 'class' &&
        getInheritableAttribute({
          entity: item,
    ...
    terms.push(
      ...getInheritableAttribute({
        ...
        .map((value) => appendNumericTerm(value))
        .flat(),
    );
...
function addMultiplierToDice(roll, multiplier) {
  ...
  return Roll.fromTerms(terms.filter((term) => !!term));
}
...
function attackDialogue(context) {
  ...
  if (context.type === 'fullAttack') {
    ...
    for (let item of equippedItems) {
      availableWeapons = Math.min(
        availableWeapons + (item.isDoubleWeapon ? 2 : 1) + (item.system.quantity > 1 ? 2 : 1),
        2,
      );
...
function getModifiersFromContextAndInputs(options, inputCriteria, modifiers) {
  ...
  options.find(inputCriteria).each((i, modifier) => {
    if (
      ((modifier.type === 'radio' || modifier.type === 'checkbox') && modifier.checked) ||
      !(modifier.type === 'radio' || modifier.type === 'checkbox')
    ) {
...
function setAttackPreviewValues(preview, attack, attackConfigOptionFields, context) {
  ...
  let damageRoll =
    `${attack.damageRoll?.renderFormulaHTML}` +
    getModifiersFromContextAndInputs(attackConfigOptionFields, '.damage-modifier', context.damageMods)
      .map((bonus) => `<span title="${bonus.source}">${bonus.value}</span>`)
      .join('');
  let attackRoll =
    `${attack.attackRoll?.renderFormulaHTML}` +
    getModifiersFromContextAndInputs(attackConfigOptionFields, '.attack-modifier', context.attackMods)
      .map((bonus) => `<span title="${bonus.source}">${bonus.value}</span>`)
      .join('');
  preview.append(
    `<div class="flex flex-col"><div>Attack Roll: <div class="attack-roll flex flex-row">${attackRoll}</div></div><div>Damage Roll: <div class="damage-roll flex flex-row">${damageRoll}</div></div>`,
  );
}
...
async function generateAttackCard(resolvedAttacks, attack) {
  ...
  for (let resolvedAttack of resolvedAttacks) {
    ...
    attackRolls += `<td><a class="inline-roll inline-result inline-dsn-hidden ${classes.join(' ')}" title="${
      resolvedAttack.attack._formula
    }" data-roll="${escape(JSON.stringify(resolvedAttack.attack.toJSON()))}"><i class="fas fa-dice-d20"></i> ${
      resolvedAttack.attack.total
    }</a></td>`;
    ...
    damageRolls += `<td><a class="inline-roll inline-result inline-dsn-hidden ${classes.join(' ')}" title="${
      resolvedAttack.damage._formula
    }" data-roll="${escape(JSON.stringify(resolvedAttack.damage.toJSON()))}"><i class="fas fa-dice-d20"></i> ${
      resolvedAttack.damage.total
    }${damageType}</a></td>`;
  }
