...
export class SWSEActorSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    html
      .find('.assignStandardArray')
      .on('click', async (event) =>
        this._selectAttributeScores(event, this, CONFIG.SWSE.Abilities.standardScorePackage, false),
      );
  ...
  async _onConditionChange(event) {
    ...
    let ids = this.actor.effects.filter((effect) => effect.icon?.includes('condition')).map((effect) => effect.id);
    ...
    let statusEffect = CONFIG.statusEffects.find(
      (e) => e.changes && e.changes.find((c) => c.key === 'condition' && c.value === event.currentTarget.value),
    );
    ...
    switch (type) {
      ...
      case 'toggle':
        let ids = this.actor.effects
          .filter((effect) => effect.icon?.includes('/shield.svg'))
  ...
  buildAgeDialog(sheet) {
    ...
    ageEffects.sort((a, b) => a.low - b.low);
  ...
  buildGenderDialog(sheet) {
    ...
    let genderEffects = filterItemsByType(sheet.actor.items.values(), 'trait')
      .filter((trait) => this._prerequisiteHasTypeInStructure(trait.data.data.prerequisite, searchString))
      .map((trait) => {
...
<div>${traits}</div>`;
  ...
  async _onDropActor(event, data) {
    ...
    if (targetItemContainer !== null) {
      ...
      if (currentPosition.length > 0) {
        ...
        if (
          !this.actor.data.data.crew.find(
            (crewMember) =>
              crewMember.position === currentPosition && crewMember.slot === targetItemContainer.dataset.slot,
          )
        ) {
  ...
  async removeReciprocalCrewLinks(actorId, positions, ev, delay = false) {
    this._pendingUpdates['data.crew'] = this.actor.data.data.crew.filter(
      (crewMember) => crewMember.id !== actorId || !positions.includes(crewMember.position),
    );
    if (!delay) {
  ...
  async removeRecipricalCrewLink(actorId, position, ev, delay = false) {
    this._pendingUpdates['data.crew'] = this.actor.data.data.crew.filter(
      (crewMember) => crewMember.id !== actorId || crewMember.position !== position,
    );
  ...
  async moveExistingItemWithinActor(data, ev) {
    if (data.modId) {
      ...
      if (equipType) {
        ...
        if (item.system.subtype.toLowerCase() === 'weapon systems') {
          let types = this.actor.system.equippedIds
            .filter((e) => e.type.startsWith('gunnerInstalled'))
            .map((e) => (e.type === 'gunnerInstalled' ? 0 : parseInt(e.type.replace('gunnerInstalled', ''))))
            .filter(unique);
          let equipType;
  ...
  _unavailable() {
    Dialog.prompt({
      ...
      content:
        "Sorry, this content isn't finished.  if you have an idea of how you think it should work please let me know.",
      callback: () => {
        return;
      },
    });
