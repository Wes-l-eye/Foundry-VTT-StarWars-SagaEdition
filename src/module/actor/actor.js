...
export class SWSEActor extends Actor {
  ...
  prepareData() {
    ...
    if (this.id && this.type === 'npc') {
      ...
      if (this._sheet instanceof SWSEManualActorSheet) {
        ...
        !this.system.attributeGenerationType ||
        this.system.attributeGenerationType.toLowerCase() === 'default'
      ) {
        this.system.finalAttributeGenerationType =
          game.settings.get('swse', 'defaultAttributeGenerationType') || 'Manual';
      }
  ...
  _prepareVehicleData(system) {
    ...
    this.cargo = filterItemsByType(this.items.values(), ['weapon', 'armor', 'equipment']).filter(
      (item) => !item.system.hasItemOwner,
    );
    ...
    for (let position of providedSlots.filter(unique)) {
      ...
      this.crewSlots.push(
        ...this.resolveSlots(
          count,
          this.system.crew.filter((crewMember) => crewMember.position === position),
          position,
          positionCover,
        ),
      );
    ...
    this.system.grapple = resolveValueArray(
      [
        this.pilot.system.offense?.bab,
        this.system.attributes.str.mod,
  ...
  _prepareCharacterData(system) {
    ...
    this.isBeast =
      !!this.classes.find((c) => c.name === 'Beast') ||
      this.naturalWeapons.length > 0 ||
  ...
  get speed() {
    ...
    for (let armor of this.getEquippedItems().filter((item) => item.type === 'armor')) {
      if (
        ...
        (armor.armorType === 'Medium' && armorType === 'Light') ||
        (armor.armorType === 'Light' && !armorType)
      ) {
        armorType = armor.armorType;
      }
    }
    return attributes
      .map((name) => this.applyArmorSpeedPenalty(name, armorType))
  ...
  getVariable(variableName) {
    return getVariableFromActorData(this, variableName);
  }
  ...
  getAttribute(attributeName) {
    return SWSEActor.getActorAttribute(this, attributeName);
  }
  ...
  get heroicLevel() {
    if (this.classes) {
      let heroicLevel = this.classes.filter((c) =>
        getInheritableAttribute({
          entity: c,
          attributeKey: 'isHeroic',
          reduce: 'OR',
   ...
   */
  isEquipable(item) {
    return item.isEquipable || (this.isDroid && item.isDroidPart) || (!this.isDroid && item.isBioPart);
  }
  ...
  _generateClassData() {
    ...
    let classSummary = Object.entries(classLevels)
    ...
    for (let bonusAttributeLevel = 4; bonusAttributeLevel < 21; bonusAttributeLevel += 4) {
      if (bonusAttributeLevel > characterLevel) {
        ...
        if (!system.levelAttributeBonus[bonusAttributeLevel]) {
          ...
          let total =
            (system.levelAttributeBonus[bonusAttributeLevel].str || 0) +
  ...
  getGunnerPositions() {
    let items = filterItemsByType(this.items.values(), 'vehicleSystem');
    let gunnerSlots = this.system.equippedIds
      .filter((id) => !!id.type && id.type.startsWith('gunnerInstalled'))
      .map((id) => parseInt(id.slot))
      .filter(unique);

    let positions = [];

    for (let gunnerSlot of gunnerSlots) {
      let gunnerItemReferences = this.system.equippedIds.filter(
        (id) => id.type.startsWith(`gunnerInstalled`) && parseInt(id.slot) === gunnerSlot,
      );
      let gunnerItems = [];
      for (let item of items) {
  ...
  getNonequippableItems() {
    return this._getUnequipableItems(SWSEActor.getInventoryItems(this.items.values())).filter(
      (i) => !i.system.hasItemOwner,
    );
  }

  static getInventoryItems(items) {
    return excludeItemsByType(
      items,
      'language',
      'feat',
      'talent',
  ...
  _getClassSkills() {
    ...
    for (let skill of skills) {
      if (
        ['knowledge (all skills, taken individually)', 'knowledge (all types, taken individually)'].includes(
          skill.toLowerCase(),
        )
      ) {
  ...
  getFirstClass() {
    ...
    if (firstClasses.length > 1) {
      console.warn(
        'more than one class item has been marked as first class on actor the first one found will be used as the first class and all others will be ignored ' +
          this.id,
      );
    }
    ...
    for (let item of actor.items) {
      ...
      if (itemSystem.isSupplied) {
        ...
        if (
          ...
          !game.settings.get('swse', 'mergePointBlankShotAndPreciseShot')
        ) {
          removal.push(item._id);
        }
      }
  ...
  _reduceProvidedItemsByExistingItems(actorData) {
    ...
    for (let provided of provides) {
      ...
      this.system.availableItems[key] = this.system.availableItems[key]
        ? this.system.availableItems[key] + value
        : value;
    }

    let classLevel = this.classes?.length;
    this.system.availableItems['General Feats'] =
      1 +
      Math.floor(classLevel / 3) +
      (this.system.availableItems['General Feats'] ? this.system.availableItems['General Feats'] : 0);
  ...
  cleanSkillName(key) {
    return this._uppercaseFirstLetters(key)
      .replace('Knowledge ', 'K')
      .replace('(', '')
      .replace(')', '')
      .replace(' ', '')
  ...
  getTooltipSections(roll) {
    ...
    for (let term of roll.terms) {
      if (term instanceof Die) {
        ...
        sections.push(
          `<section class="tooltip-part"><div class="dice">${partHeader}<ol class="dice-rolls">${diceRolls.join(
            '',
          )}</ol></div></section>`,
        );
      }
  ...
  rollOwnedItem(itemIds) {
    ...
    for (let itemId of itemIds) {
      if (itemId.provider) {
        let actor = getActorFromId(itemId.provider);
        items.push(actor.items.get(itemId.id));
        continue;
      }
      items.push(this.items.get(itemId.id));
    }
  ...
  getAvailableItemsFromRelationships() {
    ...
    if (['character', 'npc'].includes(this.type)) {
      ...
      for (let crew of this.system.crew) {
        ...
        if (vehicle) {
          let itemIds = vehicle.system.equippedIds
            .filter(
              (id) => id.position.toLowerCase() === crew.position.toLowerCase() && `${id.slot}` === `${crew.slot}`,
            )
            .map((id) => id.id);
    ...
    if (context.newFromCompendium) {
        ...
        if (!equipableTypes.includes(entity.type)) {
          if (meetsPrereqs.doesFail) {
            ...
            await Dialog.prompt({
              ...
              content:
                'You MAY meet the prerequisites. Check the remaining reqs:<br/>' +
                formatPrerequisites(meetsPrereqs.failureList),
              callback: () => {
                return;
              },
            });
      ...
      if (entity.type === 'talent') {
        ...
        if (possibleTalentTrees.size === 0) {
          await Dialog.prompt({
            ...
            content:
              "You don't have more talents available of these types: <br/><ul><li>" +
              Array.from(allTreesOnTalent).join('</li><li>') +
              '</li></ul>',
            callback: () => {
              return;
            },
      ...
      if (entity.type === 'feat' && !context.provided) {
        ...
        if (possibleFeatTypes.length === 0) {
          await Dialog.prompt({
            ...
            content:
              "You don't have more feat available of these types: <br/><ul><li>" +
              Array.from(possibleProviders).join('</li><li>') +
              '</li></ul>',
            callback: () => {
              return;
            },
      ...
      if (entity.type === 'class') {
        ...
        if (!context.skipPrerequisite && !context.isUpload) {
          if (
            entity.name === 'Beast' &&
            !context.isFirstLevel &&
            this.classes.filter((clazz) => clazz.name === 'Beast').length === 0
          ) {
            new Dialog({
    ...
    if (!context.isUpload) {
      if (context.isFirstLevel) {
        if (availableClassFeats > 0 && availableClassFeats < feats.length) {
          ...
          let featString = await this.addItems(
            feats.map((feat) => {
  ...
  async addItems(items, parent, options = {}) {
    ...
    let addedItems = [];
    for (let providedItem of items.filter(
      (item) =>
        item && ((item.name && item.type) || (item.uuid && item.type) || (item.id && item.pack) || item.duplicate),
    )) {
  ...
  async equipItem(itemId, equipType, options) {
    ...
    if (!!options.offerOverride) {
      ...
      if (meetsPrereqs.doesFail) {
        await new Dialog({
          title: "You Don't Meet the Prerequisites!",
          content: `You do not meet the prerequisites for equipping ${v.finalName}:<br/> ${formatPrerequisites(
            meetsPrereqs.failureList,
          )}`,
          buttons: {
            ok: {
              icon: '<i class="fas fa-check"></i>',
      ...
      if (meetsPrereqs.doesFail) {
        new Dialog({
          title: "You Don't Meet the Prerequisites!",
          content: `You do not meet the prerequisites for equipping ${
            item.system.finalName
          }:<br/> ${formatPrerequisites(meetsPrereqs.failureList)}`,
          buttons: {
            ok: {
              icon: '<i class="fas fa-check"></i>',
              label: 'Ok',
  ...
  async resolveUpdate(itemId, equipType, slot, position) {
    let update = {};
    update['system.equippedIds'] = [{ id: itemId, type: equipType, slot, position }].concat(
      this.system.equippedIds.filter((value) => !!value && value !== itemId && value?.id !== itemId),
    );
  ...
  isPermittedForActorType(type) {
    if (['character', 'npc'].includes(this.type)) {
      ...
      return ['weapon', 'armor', 'equipment', 'upgrade', 'vehicleBaseType', 'vehicleSystem', 'template'].includes(type);
    }
