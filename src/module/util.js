...
function resolveFunction(expression, deepestStart, deepestEnd, func, actor) {
  ...
  if (preceeding.endsWith(func.name)) {
    ...
    return resolveExpression(
      expression.substring(0, deepestStart - func.name.length) + result + expression.substring(deepestEnd + 1),
      actor,
    );
  }
}
...
function resolveParensAndFunctions(expression, actor) {
  ...
  // }
  let resolveExpression1 = resolveExpression(
    expression.substring(0, deepestStart) +
      resolveExpression(expression.substring(deepestStart + 1, deepestEnd), actor) +
      expression.substring(deepestEnd + 1),
    actor,
  );
...
export function resolveAttackRange(effectiveRange, distance, accurate, inaccurate) {
  ...
  let resolvedRange = Object.entries(range).filter(
    (entry) => entry[1].low <= distance && entry[1].high >= distance,
  )[0][0];
  ...
  for (let select of selects) {
    ...
    for (let o of select.options) {
      if (o.value !== '--' && !o.selected) {
        ...
        if (
          selected.options.tripleAttack &&
          (!hasDoubleAttack ||
            (hasTripleAttack && !selectValue.options.tripleAttack) ||
            selectValue.options.doubleAttack)
        ) {
          o.disabled = true;
        }
      }
...
export function getAttackRange(range, isAccurate, isInaccurate, actor) {
  ...
  let sources =
    Object.values(canvas.tokens.controlled).filter((token) => token.document.actorId === (actor._id || actor.id)) || []; //get selected tokens of this actor
  if (sources.length === 0) {
    sources =
      canvas.tokens.objects?.children.filter((token) => token.document.actorId === (actor?._id || actor?.id)) || [];
  }
...
export function reduceArray(reduce, values, actor) {
  ...
  switch (reduce) {
    case 'SUM':
      return values
        .map((attr) => resolveValueArray(attr.value, actor))
        .reduce((a, b) => (a === 0 && isNaN(b) ? b : a + b), 0);
    case 'AND':
    ...
    case 'MIN':
      return values
        .map((attr) => toNumber(attr.value))
        .reduce((a, b) => (a === undefined ? b : Math.min(a, b)), undefined);
    case 'FIRST':
