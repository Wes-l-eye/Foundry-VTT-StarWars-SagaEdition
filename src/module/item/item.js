...
export class SWSEItem extends Item {
  ...
  get isBioPart() {
    return (
      this.subType === 'Implants' ||
      this.subType === 'Bio-Implants' ||
      this.subType === 'Cybernetic Devices' ||
      this.subType === 'Advanced Cybernetics'
    );
  }

  get isDroidPart() {
    return (
      this.subType === 'Locomotion Systems' ||
      this.subType === 'Processor Systems' ||
      this.subType === 'Droid Templates' ||
      this.subType === 'Appendages' ||
      this.subType?.startsWith('Droid Accessories')
    );
  }
  ...
  setStripping(key, label, enabled, type, low, high) {
    ...
    this.system.stripping[key].value = enabled
      ? this.system.stripping[key].value || (type === 'boolean' ? false : type === 'string' ? '' : 0)
      : false;
  ...
  getStripping(key) {
    return SWSEItem.getItemStripping(this, key);
  }
  ...
  prepareArmor(system) {
    ...
    this.setStripping(
      'reduceDefensiveMaterial',
      'Reduce Defensive Material',
      defensiveMaterial > 0,
      'number',
      0,
      defensiveMaterial,
    ...
    this.setStripping(
      'reduceJointProtection',
      'Reduce Joint Protection',
      jointProtection > 0,
      'number',
      0,
      jointProtection,
  ...
  canReduceRange() {
    ...
    return (
      subtypes.includes(this.system.subtype?.toLowerCase()) ||
      subtypes.includes(this.system.attributes?.treatedAsForRange?.toLowerCase())
    );
  }
  ...
  reduceRange(finalWeaponRange) {
    ...
    let index = ranges.indexOf(
      finalWeaponRange === 'Simple Ranged Weapon'
        ? 'Pistols'
        : finalWeaponRange === 'Grenades'
        ? 'Thrown'
        : finalWeaponRange,
    );
  ...
  static getChildModes(activeModes, parentModePath) {
    ...
    for (let activeMode of activeModes) {
      ...
      childModes.push(
        ...SWSEItem.getChildModes(
          values.filter((mode) => !!mode && mode.isActive),
          parentModePath1,
        ),
      );
    }
  ...
  _activateMode(modes, mode, data) {
    ...
    if (modeTokens.length === 1) {
      let groups = Object.values(modes || [])
        .filter((m) => !!m && m.name === mode)
        .map((m) => m.group)
        .filter((g) => !!g);
      ...
      if (groups.length > 0) {
        groups.forEach((group) => {
          Object.entries(modes || [])
            .filter((entity) => entity[1]?.group === group)
            .forEach((entity) => {
        ...
        Object.entries(modes || [])
          .filter((entity) => !!entity[1] && entity[1].name === mode)
          .forEach((entity) => {
            data.modes[parseInt(entity[0])] = {};
      ...
      Object.entries(modes || [])
        .filter((entity) => entity[1]?.isActive && entity[1]?.name === first)
        .forEach((entity) => {
  ...
  deactivateMode(mode) {
    let update = {};
    update.data = {};
    update.data.activeModes = this.system.activeModes.filter(
      (activeMode) => activeMode.toLowerCase() !== mode.toLowerCase(),
    );
    this.safeUpdate(update);
  }
