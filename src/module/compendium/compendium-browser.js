...
export class SWSECompendiumBrowser extends Application {
   ...
   */
  _onDragStart(event) {
    const packName = this.getAttribute('data-collection'),
  ...
  _determineFilteredItemCount() {
    ...
    this.element.find('span[data-type="filterItemCount"]').text(itemCount); //game.i18n.localize("PF1.FilteredItems").format(itemCount));
  }

  _passesFilters(item) {
    let matchesProviderGroup = item.groupTypes
      .map((type) => {
    ...
    if (
      !this.filterQuery.test(item.name) &&
    ...
    for (let key of Object.keys(groupedFilters)) {
      if (!groupedFilters[key].map((f) => f.test(item)).reduce((previous, next) => previous || next, false))
        return false;
    }
